...
struct sherwood_v8_block {
  ...
  int first_empty_index() const {
    for (int i = 0; i < BlockSize; ++i) {
      if (control_bytes[i] == sherwood_v8_constants<>::magic_for_empty) return i;
    }
    return -1;
  }
...
class sherwood_v8_table : private ByteAlloc, private Hasher, private Equal {
 ...
 public:
  ...
  explicit sherwood_v8_table(size_type bucket_count, const ArgumentHash &hash = ArgumentHash(),
      ...
      , Hasher(hash)
      , Equal(equal) {
    if (bucket_count) rehash(bucket_count);
  }
  ...
  struct templated_iterator {
   ...
   public:
    ...
    friend bool operator==(const templated_iterator &lhs, const templated_iterator &rhs) { return lhs.index == rhs.index; }
    friend bool operator!=(const templated_iterator &lhs, const templated_iterator &rhs) { return !(lhs == rhs); }

    templated_iterator &operator++() {
      do {
        if (index % BlockSize == 0) --current;
        if (index-- == 0) break;
  ...
  inline iterator find(const FindKey &key) {
    ...
    for (;;) {
      ...
      if (first) {
        if ((metadata & Constants::bits_for_direct_hit) != Constants::magic_for_direct_hit) return end();
        first = false;
      }
      if (compares_equal(key, block->data[index_in_block])) return {block, index};
      int8_t to_next_index = metadata & Constants::bits_for_distance;
      if (to_next_index == 0) return end();
  ...
  inline std::pair<iterator, bool> emplace(Key &&key, Args &&... args) {
    ...
    for (;;) {
      ...
      if (compares_equal(key, block->data[index_in_block])) return {{block, index}, false};
      int8_t to_next_index = metadata & Constants::bits_for_distance;
      if (to_next_index == 0) return emplace_new_key({index, block}, std::forward<Key>(key), std::forward<Args>(args)...);
      index += Constants::jump_distances[to_next_index];
      index = hash_policy.keep_in_range(index, num_slots_minus_one);
    }
  }
  ...
  void rehash(size_t num_items) {
    ...
    if (num_items == num_slots_minus_one + 1) return;
    size_t num_blocks = num_items / BlockSize;
    if (num_items % BlockSize) ++num_blocks;
    ...
    if (bucket_count() && num_items > 16) {
      LOG(__PRETTY_FUNCTION__, " rehashed to num_items=", num_items, " num_blocks=", num_blocks, " mem=", memory_requirement, "\n");
    ...
    for (BlockPointer it = new_buckets; it <= special_end_item; ++it) it->fill_control_bytes(Constants::magic_for_empty);
    using std::swap;
    ...
    if (num_items % BlockSize) ++old_num_blocks;
  ...
  convertible_to_iterator erase(const_iterator to_erase) {
    ...
    if (current.has_next()) {
      ...
      if (!current.is_direct_hit()) find_parent_block(current).clear_next();
      AllocatorTraits::destroy(*this, std::addressof(*current));
  ...
  iterator erase(const_iterator begin_it, const_iterator end_it) {
    if (begin_it == end_it) return {begin_it.current, begin_it.index};
    if (std::next(begin_it) == end_it) return erase(begin_it);
  ...
  void swap(sherwood_v8_table &other) {
    ...
    if (AllocatorTraits::propagate_on_container_swap::value) swap(static_cast<ByteAlloc &>(*this), static_cast<ByteAlloc &>(other));
  }
 ...
 private:
  ...
  struct LinkedListIt {
    ...
    bool is_direct_hit() const { return (metadata() & Constants::bits_for_direct_hit) == Constants::magic_for_direct_hit; }
  ...
  LinkedListIt find_parent_block(LinkedListIt child) {
    ...
    for (;;) {
      ...
      if (next == child) return parent_block;
      parent_block = next;
    }
  }
  ...
  std::pair<int8_t, LinkedListIt> find_free_index(LinkedListIt parent) const {
    for (int8_t jump_index = 1; jump_index < Constants::num_jump_distances; ++jump_index) {
      ...
      if (block->control_bytes[index % BlockSize] == Constants::magic_for_empty) return {jump_index, {index, block}};
    }
    return {0, {}};
  }
  ...
  void deallocate_data(BlockPointer begin, size_t num_slots_minus_one) {
    if (begin == BlockType::empty_block()) return;

    ++num_slots_minus_one;
    size_t num_blocks = num_slots_minus_one / BlockSize;
    if (num_slots_minus_one % BlockSize) ++num_blocks;
...
class bytell_hash_map : public detailv8::sherwood_v8_table<std::pair<K, V>, K, H, detailv8::KeyOrValueHasher<K, std::pair<K, V>, H>,
  ...
  using Table = detailv8::sherwood_v8_table<std::pair<K, V>, K, H, detailv8::KeyOrValueHasher<K, std::pair<K, V>, H>, E,
...
class bytell_hash_set
    : public detailv8::sherwood_v8_table<T, T, H, detailv8::functor_storage<size_t, H>, E, detailv8::functor_storage<bool, E>, A,
                                         typename std::allocator_traits<A>::template rebind_alloc<unsigned char>,
